...
Shape* parserTriangleCommand(stringstream &); 
Shape* parserRectangleCommand(stringstream &); 

int main () {

    // Register the command parser call back functions
    sdb.registerShapeType("circle", &parseCircleCommand);
    sdb.registerShapeType("triangle", &parserTriangleCommand);
    sdb.registerShapeType("rectangle", &parserRectangleCommand);
...
Shape* parserRectangleCommand(stringstream& line) {
    string name;
    float xcent;
    float ycent;
    float width;
...
Shape* parserTriangleCommand(stringstream& line) {
    string name;
    float xcent;
    float ycent;
    ...
    ycent = (y1+y2+y3)/3; 
    // Create the shape object and return a pointer to it
    Shape* myShape = (Shape*) new Triangle(name, xcent, ycent, x1, x2, x3, y1, y2, y3);
